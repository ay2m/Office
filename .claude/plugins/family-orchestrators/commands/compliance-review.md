...
- **PDPL and ZATCA are the governing regimes.** The vendored skills in
  `iflygaca/Office/.claude/skills/` (GDPR controls, privacy impact assessment,
...
In `iflygaca/FlyGACA` the disclaimer is a component (`<Disclaimer />`) and must
never be inlined or reworded. In `iflygaca/Captain-Adel` the `.disclaimer-strip` is
...
So: flag every place — repo docs, `iflygaca/Office`, investor decks — that states
in-Kingdom residency as **present fact**. Aspirational is fine when labelled;
unlabelled is a compliance claim the family cannot support. `iflygaca/Office`'s
`06-operations-it/runbooks/runbook-pdpl-me-central2.md` and
`04-compliance-ksa/` hold the KSA side; `docs/RUNBOOK-golive.md` §5 in
`iflygaca/FlyGACA` holds the sub-processor disclosure.

## PDPL, operationally

- Real user questions are personal data. In `iflygaca/Captain-Adel` the chat model
...
- Erasure must exist for every user-keyed store. `POST /v1/account/delete`
  ...
  In `iflygaca/FlyGACA`, a new user-keyed table needs its erasure path in the same
  change.
...
E-invoicing lives in `iflygaca/Office`: `03-finance/` holds the tax-invoice and
