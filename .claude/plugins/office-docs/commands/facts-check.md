...
`ay2m/Office` **owns** the `entity` block of `contracts/flygaca-family.json`.
...
manifest travels byte-identically to `ay2m/FlyGACA` and `ay2m/Captain-Adel`.
...
5. Open all three PRs together, and check the consumers still carry the strings
   verbatim: `ay2m/FlyGACA`'s `src/lib/seo/jsonld.ts` plus `footer.legalEntity`
   and `legal.*` in both i18n bundles; `ay2m/Captain-Adel`'s `footer.js`,
