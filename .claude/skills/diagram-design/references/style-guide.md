...
Skinned to the **Fly GACA Falcon palette** — ivory paper, falcon-night ink, falcon-teal accent, slate muted. Values mirror the Falcon tokens; change them there first, then mirror here.
...
|---|---|---|---|
| `paper` | Page background, default node fill | `#f5f2ed` (ivory) | `#0a0e12` (falcon-night) |
| `paper-2` | Diagram container bg, secondary fill | `#fbf8f2` (ivory-warm) | `#0f1a24` (falcon-deep) |
| `ink` | Primary text, primary stroke | `#0a0e12` (falcon-night) | `#e8edf2` (text) |
| `muted` | Secondary text, default arrow stroke | `#3f4a55` (ink-soft) | `#9da9b4` (text-muted) |
| `soft` | Sublabels, boundary labels | `#7a8590` (ink-mute) | `#8a95a1` (text-dim) |
| `rule` | Hairline borders | `rgba(10,14,18,0.12)` | `rgba(232,237,242,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#e5dfd3` (ivory-edge) | `#1a2a38` (falcon-mist) |
| `accent` | Focal / 1–2 max per diagram | `#2d6e8a` (falcon-teal) | `#4a9cb8` (teal-bright) |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(45,110,138,0.08)` | `rgba(74,156,184,0.10)` |
| `link` | HTTP/API calls, external arrows | `#c8a04a` (falcon-gold) | `#e0c588` (gold-soft) |

> **Brand palette source:** these tokens are the **Falcon palette**, defined canonically in
> `src/styles/tokens.css` (Fly GACA app) and mirrored in `11-brand/design-token-map-*.md` (Office)
> and `FeatureUI/Theme.swift` (iOS). `accent` maps to `--falcon-teal` — the brand primary, so a
> diagram reads as Fly GACA at a glance. `link` maps to `--falcon-gold`, the heritage accent the
> brand reserves for sparing use, which suits external/API arrows: a distinct warm hue that stays
> rare by nature. `soft`, `rule`, and `rule-solid` are derived (ink-at-opacity and the ivory/mist
> border steps) to cover roles the five-colour palette doesn't name directly.
