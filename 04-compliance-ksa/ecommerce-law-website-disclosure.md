...
|---|---|---|
| flygaca.com | Footer identity line (every page) | `iflygaca/FlyGACA: src/i18n/en.json` + `ar.json` key `footer.legalEntity`, rendered by `src/app/Footer.tsx` |
| flygaca.com | Terms §1 entity block, Privacy §1 (PDPL controller) | `iflygaca/FlyGACA: src/i18n/*.json` under `legal.terms` / `legal.privacy` |
| flygaca.com | Machine-readable (search/AI) | Organization JSON-LD: `legalName`, `PostalAddress`, SA-CR `identifier`, `vatID` — `iflygaca/FlyGACA: src/lib/seo/jsonld.ts`, `index.html`, `scripts/prerender-head.mjs` |
| captadel.com | Footer identity line (all five pages) | `iflygaca/Captain-Adel: public/assets/js/footer.js` (`.footer-entity`) |
| captadel.com | `/terms` + `/privacy` entity blocks | `iflygaca/Captain-Adel: public/terms.html`, `public/privacy.html` |
